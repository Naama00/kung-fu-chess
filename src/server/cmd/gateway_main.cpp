...
namespace kungfu
{
    ...
    void runGatewayService()
    {
        ...
        // 1. Initialize PostgreSQL User Repository for Client Auth
        auto userRepository = std::make_shared<PostgresUserRepository>();
        std::string postgresConn = ServerConfig::getPostgresConn();
        if (!userRepository->initialize(postgresConn))
        {
            std::cerr << "[Gateway] Critical: PostgreSQL User Repository init failed!" << std::endl;
            return;
        }
        ...
        std::cout << "[Gateway PubSub] Broadcast received: " << payload << std::endl; }, redisHost, redisPort);
